...
const App: React.FC = () => {
  ...
  const getWeatherGradient = (code: number) => {
    if (code === 0) return 'from-amber-400 via-orange-500 to-rose-500';
    if (code >= 1 && code <= 3) return 'from-sky-400 via-blue-500 to-indigo-600';
    if (code >= 45 && code <= 48) return 'from-slate-400 via-slate-500 to-slate-700';
    if (code >= 51 && code <= 65) return 'from-blue-600 via-indigo-700 to-slate-900';
    if (code >= 71 && code <= 86) return 'from-teal-400 via-cyan-600 to-blue-800';
    if (code >= 95) return 'from-purple-800 via-slate-900 to-black';
